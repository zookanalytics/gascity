...
#      captured immediately before the mutating reset
...
#      during/after verify). All other failures — and gain+drift or
#      row-decrease with a stable HEAD — still quarantine. Probe failure
#      leaves the race unproven and quarantines.
...
flatten_database() {
  ...
  if [ "$verify_counts_rc" -ne 0 ]; then
    ...
    # where preservation is proven, rather than assuming corruption. Three
    # proof-gated defer arms follow: gain+drift with a HEAD-proven writer
    # race; any table value-hash drift (with or without a row-count gain)
    # proven by the removal-based DOLT_DIFF probe to have removed no
    # pre-flight row (step 4b); and row-count decrease with a HEAD-proven
    # concurrent DELETE. Table-list drift, probe failure, or any case whose
    # specific proof fails still quarantine below unchanged.
