...
// health command. These tests pin its contract: the check measures the
// scheduled backup destinations ($GC_BACKUP_ARTIFACT_DIR/<db>, default
...
func (f *backupFixture) run(t *testing.T) backupsReport {
	...
	out, err := newHealthScriptCmd(f.root, env, "--json").Output()
	if err != nil {
...
// to measure, so the aggregate is null (could-not-measure), never false. An
// unmeasured backup must not read clean.
...
// TestHealthBackupsIgnoresMigrationBackupSnapshot asserts that a fresh
// migration-backup-* rollback snapshot must NOT make a database whose real
// destination is missing read as clean. The check measures the scheduled
// destination, not the rollback snapshot, so an absent destination reads stale
// whatever snapshot is present.
func TestHealthBackupsIgnoresMigrationBackupSnapshot(t *testing.T) {
	...
	// A fresh rollback snapshot, which the check must not treat as a backup.
