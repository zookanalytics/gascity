...
case "$query" in
  ...
  *"DOLT_HASHOF_TABLE('beads')"*)
    ...
    if { [ "$mode" = "same_row_count_writer" ] || [ "$mode" = "same_row_count_row_loss" ] || [ "$mode" = "writer_race_same_count_hash_drift_only" ] || [ "$mode" = "writer_race_same_count_hash_drift_diff_fails" ]; } && [ "$(current_head)" = "compactcommit" ]; then
      print_cell hash-beads-after-writer
      exit 0
    fi
  ...
  *"DOLT_DIFF("*"diff_type = 'removed'"*)
    ...
    # also match this query's text. Matching the diff_type = 'removed' clause
    # rather than DOLT_DIFF alone keeps this clear of the additive-only probe
    # on a first-committed table, which asks diff_type <> 'added' and is
    # answered by its own arm further down.
    #
    ...
    case "$mode" in
      same_table_replacement_with_row_gain|writer_race_with_mixed_same_count_hash_drift|same_row_count_row_loss|writer_race_same_count_hash_drift_diff_fails)
        print_cell 2
        ;;
      *)
        print_cell 0
...
// row count. Both were ordinary traffic — DOLT_DIFF across the recorded HEADs
// showed zero removals in either table. Every defer arm excluded same-count
// drift categorically, so this quarantined and cost a human review. With no
// rows removed, the flatten preserved everything it was asked to preserve:
// defer and retry next run.
...
func assertCompactWriterRaceDeferred(t *testing.T, fixture compactScriptFixture, out string, err error) {
	...
	assertCompactDeferMarkers(t, fixture, out)
}

// assertCompactDeferMarkers pins the state every deferral leaves behind,
// independent of which proof produced it: no quarantine marker, a pending-GC
// retry marker, and no DOLT_GC this run. The row-preservation proof
// (drift_preserves_preflight_rows) defers without a HEAD fingerprint, so it
// reuses these checks rather than the writer-race message above.
func assertCompactDeferMarkers(t *testing.T, fixture compactScriptFixture, out string) {
	t.Helper()
	quarantine := filepath.Join(fixture.cityPath, ".gc", "runtime", "packs", "dolt", "compact-quarantine", "beads")
...
func TestCompactScriptDefersProvenWriterRaceSameCountHashDrift(t *testing.T) {
	...
	if !strings.Contains(out, "value-hash drift removed no pre-flight rows, proven via DOLT_DIFF") {
		t.Fatalf("defer message should report the row-preservation proof:\n%s", out)
	}
	if err != nil {
		t.Fatalf("row-preservation defer must exit 0 (skip, not failure): %v\n%s", err, out)
	}
...
// Production incident (lx 2026-08-09 and again 2026-08-15, gc-800l/gc-i52hj):
// a concurrent `bd update` rewrites an existing row in place. The row count is
// unchanged and the table value hash drifts — the signature of the single most
// common write in a running city, which the check used to treat as unexplained
// and quarantine on, with no proof path at all. Every defer arm was guarded off
// by that one category, so an unattended 24h timer generated a human review per
// pass and never reclaimed; the second incident blocked reclaim for 4 days and
// let one database reach 7-11x its peers. An in-place update removes nothing,
// so the removal-based proof settles it directly: defer and retry next run.
