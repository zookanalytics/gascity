...
agent from cost telemetry. Both are fixed here.
...
- **In-interval model-usage sweep** — the controller tick now sweeps sessions
  whose interval is still open, on a 10-minute cadence, gated by a new
  `usage_model_live_swept_at` cadence marker. It deliberately does not stamp the
  end-of-interval marker and does not mint a compute fact; the two lanes
  partition on terminal state and the persisted invocation-usage cursor keeps
  re-swept invocations single-counted.
