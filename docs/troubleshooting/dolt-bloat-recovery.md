...
postflight database value hashes, `integrity_table_drift` for table-level
...
| Value | Meaning |
|-------|---------|
| a hash | The probe ran and returned this value. |
| `<empty>` | The probe ran and returned nothing. |
| `<probe-failed>` | The probe was attempted and errored. |
| `<not-measured>` | The run quarantined before reaching this probe. |
