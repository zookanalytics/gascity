...
// interaction, then attachment, then assigned work, then min_active_sessions
// floor membership, then stop. A pending interaction cancels any pending drain
// and keeps the session out of this tick's wake pass — an asymmetry with
// max-session-age that is part of the existing reconciler contract. Attachment
// defers the stop on the same footing as a pending interaction: idle is measured
// from provider output activity, which cannot observe a human *reading* an
// attached pane, so without this rung a session someone is actively watching is
// indistinguishable from an abandoned one and gets reaped mid-attention
// (gc-rjtk1). It also mirrors the assigned-work rung's fight against the
// idle-kill/wake treadmill (ga-3ox7rk): ComputeAwakeSet already re-wakes an
// attached session ("attached" wake override), so killing it here would only
// bounce it straight back awake. The defer is re-evaluated every tick, so the
// stop resumes as soon as the terminal detaches — it is not an unconditional
// exemption, and unlike the pending arm it does not cancel drains or skip the
// wake pass. Assigned work defers the stop, mirroring DecideMaxSessionAge:
...
// treadmill (ga-3ox7rk). A floor member with no assigned work defers too: it is
// kept WARM rather than idle-killed and cold-recreated by the min-fill next tick
// (sc-5mtyhy). The floor rung sits AFTER assigned work so a busy floor session
// still defers as assigned_work (and feeds the same-bead exhausted backstop);
// only a genuinely idle floor member reaches the keep-warm defer.
