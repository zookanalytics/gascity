...
func ensureSessionNameAvailableForSelfAndOwner(store beads.Store, name, selfID, selfOwner string) error {
	...
	for _, b := range all {
		...
		if strings.TrimSpace(b.Metadata["session_name"]) == name {
			...
			//
			// Pool-managed beads are excluded so the pool keeps its own,
			// stricter rule below (pool_managed AND ephemeral). This recognizer
			// matches on template as well as identity/alias/agent_name, and a
			// pool slot carries the pool's template — so without this exclusion
			// a closed pool bead missing EITHER pool marker would be released
			// here by the identity lane, silently widening the pool carve-out
			// that upstream deliberately keyed on both markers.
			if b.Status == "closed" &&
