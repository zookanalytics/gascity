...
// [[agent]] block with start_command (no provider) carries process_names
// through so the runtime-readiness check has a non-empty list to match
// against. Without this, IsRuntimeRunning short-circuits to false on the
// first probe (its empty-list guard), the bead never confirms
// creation_complete, and the reconciler eventually rolls state=creating
// back to state=asleep while pending_create_claim stays set — stranding
// the agent for ~34 minutes (gc-baysm).
...
// that an [[agent]] block with start_command can opt into permission-warning
