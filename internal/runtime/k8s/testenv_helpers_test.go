...
// GC_STORE_ROOT env vars for the duration of the test so the child scripts
// spawned via runControllerScriptDeploy and runBeadsScript (which inherit the
// test process's env through `os.Environ()`) do not observe a leak from the
// developer's shell or an enclosing gc city. Each test's opts.Env continues to
// declare its own desired state, which overrides the emptied values when
// cmd.Env is flattened.
//
// GC_STORE_ROOT in particular leaks whenever the suite runs inside a gc city
// (CI, polecat worktrees), where it points at the enclosing city root. Tests
