...
# `printf ... | grep -Fxq` pipeline that d416a0085 (reaper.sh) and an earlier
# fix (here) removed: under `set -o pipefail` grep -q's early exit SIGPIPEs
# the writer and a present candidate reads as absent. The pure-bash test also
