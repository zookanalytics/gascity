...
//     observation in session.Manager.infoFromBead and the State() probe in
//     enrichSessionResponse (the ~2N uncached tmux forks per request this cache
//     eliminates).
//   - state carries the live active->asleep downgrade infoFromBead applies, so
...
// ListFullFromBeads + enrichSessionResponse path view=full used to run inline on
// every request — for the active sessions and projects each result down to its
// live fields. This is where the per-session tmux forks happen; the warm cache
// confines them to one sweep per TTL window instead of one per request. Returns
// nil on a store read error so the caller can keep the previous snapshot.
func (s *Server) computeSessionLiveFields() map[string]sessionLiveFields {
	store := s.state.CityBeadStore()
	if store == nil {
		return nil
	}
	all, _, err := sessionReadModelRows(store)
	if err != nil {
		return nil
	}
	cfg := s.state.Config()
	mgr := s.sessionManager(store)
	// Only active sessions carry live fields: infoFromBead probes the runtime
	...
	listResult := mgr.ListFullFromBeads(all, string(session.StateActive), "")
	fields := make(map[string]sessionLiveFields, len(listResult.Sessions))
