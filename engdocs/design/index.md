...
| `templated-overlay-staging` | Implemented | `<name>.template.<ext>` overlay files rendered during staging against the shared pack template data map, so install-specific values (CityRoot) bind at staging time instead of by a post-hoc rewrite |
