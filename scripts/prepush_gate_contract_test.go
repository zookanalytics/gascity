...
func setupPrePushRepo(t *testing.T) (string, string) {
	...
	for _, f := range []struct{ src, dst string }{
		...
		// The hook chains beads' own pre-push through this lib before its
		// Go-change scan, so the fixture needs the real copy beside it.
		{filepath.Join(root, ".githooks", "lib", "beads-chain.sh"), filepath.Join(repo, ".githooks", "lib", "beads-chain.sh")},
...
func runPrePush(t *testing.T, repo, stdin string) (string, error) {
	t.Helper()
	// The beads chain the hook runs first shells out to `bd`; a no-op stub
	// ahead of the host PATH keeps a real bead store out of this contract.
	binDir := t.TempDir()
	if err := os.WriteFile(filepath.Join(binDir, "bd"), []byte("#!/usr/bin/env sh\nexit 0\n"), 0o755); err != nil {
