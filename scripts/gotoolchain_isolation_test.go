...
// GOROOT/GOTOOLCHAIN pairing it exists to enforce. `\b` matches between the
...
// TestGoEnvAllowlistsForwardGoToolchainWithGoroot asserts every `env -i`
// allowlist that forwards GOROOT also forwards GOTOOLCHAIN.
//
		...
		t.Run(rel, func(t *testing.T) {
			...
			if !forwardsGotoolchain.MatchString(content) {
				t.Errorf("%s forwards GOROOT but not GOTOOLCHAIN through its `env -i` allowlist.\n"+
					...
					"\"version go1.X does not match go tool version go1.Y\" under the agent pre-commit hook.", rel)
			}
		})
	}
}
