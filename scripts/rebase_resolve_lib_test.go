...
// `writer | grep -q` SIGPIPE false-negative defect class (following the
// pack-script and orphan-sweep fixes, and d416a0085). `grep -q` exits the
// instant it matches without draining stdin, so a writer piped into it races
// into a SIGPIPE; under a caller's `set -o pipefail` that 141 becomes the
// pipeline's exit status and a PRESENT match is reported as ABSENT. Use git
// grep's own exit status, capture-then-test, or a here-string
// (`grep -q ... <<<"$x"`) instead.
//
