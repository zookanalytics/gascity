...
func resolveTemplate(p *agentBuildParams, cfgAgent *config.Agent, qualifiedName string, fpExtra map[string]string) (TemplateParams, error) {
	...
	//
	// workspaceEnv, resolved.Env (provider preset env merged with
	// agent.Env via mergeAgentOverrides), and cfgAgent.Env (agent.toml
	// [env] only) all expand against the same source. Cross-layer
	// expansion (cfgAgent.Env referencing already-expanded resolved.Env
	// values) is deliberately not supported here: because
	// mergeAgentOverrides already folds agent.Env into resolved.Env, a
	// layered expansion would re-expand references like "${PATH}"
	// against a PATH that already contains the agent's own augmentation,
	// producing duplicated segments.
	//
	...
	env := mergeEnv(pthru, expandEnvMap(expansionSrc, workspaceEnv), expandEnvMap(expansionSrc, resolved.Env), expandEnvMap(expansionSrc, cfgAgent.Env), agentEnv)
