...
func (m *memoryOrderDispatcher) dispatch(ctx context.Context, cityPath string, now time.Time) {
	...
	for offset := 0; offset < total; offset++ {
		...
		if a.Trigger == "event" {
			...
			// next tick. A bead cursor would emit bead.updated and self-trigger
			// orders that watch bead.* events (gc-9hxxv).
			if m.ep == nil {
				continue
			}
			...
			trackingRun, err := orders.NewStore(beads.OrdersStore{Store: store}).CreateRun(scoped, orders.RunOpts{})
			if err != nil {
				logDispatchError(m.stderr, "gc: order dispatch: creating tracking bead for %s: %v", scoped, err)
				continue
			}
			trackingID = trackingRun.ID
			m.rememberLastRun(scoped, storeKeysForGate, trackingRun.CreatedAt)
		}
...
func (m *memoryOrderDispatcher) markTrackingFailure(store beads.Store, trackingID, scoped string) {
	...
	if err := store.Update(trackingID, beads.UpdateOpts{Labels: []string{"wisp", "wisp-failed"}}); err != nil {
