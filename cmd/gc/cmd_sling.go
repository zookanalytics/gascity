...
func (r cliBeadRouter) Route(_ context.Context, req sling.RouteRequest) error {
	...
	// Target-type-conditional single-stamp (Path D). A singleton target (no
	// pool instance expansion) gets a direct assignee stamp ONLY: its own
	// work_query short-circuits Tiers 2-3 for named-origin sessions, so only a
	// Tier 1 assignee match surfaces routed work on the target's hook. A pool
	...
	// not dual-stamp (upstream PR #1736). See gastownhall/gascity#yb5uhi.
	if req.Singleton {
		target := req.Target
