...
// durable event cursor (consumer-offset) before the command and mint no
// tracking bead — the per-fire no-op tracking beads otherwise churned ~6k/day
// via nudge-on-route (gc-9hxxv). Cooldown/cron orders have no cursor, so they
// record a closed tracking bead that `gc order check` reads to advance the
// cooldown clock (#3570).
func doOrderRunExecTracked(a orders.Order, cityPath string, cfg *config.City, store beads.Store, ep events.Provider, stdout, stderr io.Writer) int {
	...
	// Event orders: advance the cursor before the side effect so a crash can't
	// leave it stale and replay the same event window on restart. No bead.
	if a.Trigger == "event" && ep != nil {
		headSeq, err := ep.LatestSeq()
		if err != nil {
	...
	// Cooldown/cron: record a closed tracking bead so the run is visible to the
	// labelOrderTracking history index and the order does not re-fire every tick
	// (#3570). Closed on creation: a lingering open bead reads as in-flight work
	// and blocks re-dispatch.
...
func cmdOrderSweepTrackingWithOptions(staleAfter time.Duration, includeWisps, dryRun, quiet bool, orderNames []string, stdout, stderr io.Writer) int {
	...
	stores, openErr := orderTrackingSweepStoresForConfigTargets(cityPath, cfg, requiredTargets, stderr)
	if len(stores) == 0 {
		if openErr != nil {
