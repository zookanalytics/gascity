#!/bin/sh
# Unit test for the post-flatten row-preservation proofs (#2846, generalized to
# removal-based for the drift path in gc-800l/gc-i52hj).
...
#
# Two predicates are covered because run.sh asks two different questions:
# drift_preserves_preflight_rows (did the flatten drop a row it was asked to
# preserve — removals only) and diff_is_additive_only (a table the flatten
# first-committed, which has no pre-flight rows, so anything but an add is
# unexplained). Collapsing them would re-break the case gc-800l fixed.
set -u
...
# query and echo the canned count stub_count_<table> (default 0) — read as
# removed rows or as non-added rows depending on which predicate is calling.
...
# --- drift_preserves_preflight_rows: removals decide ------------------------
...
# in-place UPDATE (the lx production case: issues modified=1, removed=0) leaves
...
if grep -q "diff_type = 'removed'" "$QUERY_LOG"; then
  ok "drift proof counts removed rows only"
else
  no "drift proof counts removed rows only (queries were: $(cat "$QUERY_LOG"))"
fi

# --- diff_is_additive_only: the first-committed table keeps a stricter bar ---

# 12. purely additive first-commit diff -> provable
reset; stub_count_issues=0
if diff_is_additive_only db H1 H2 "issues"; then ok "additive-only first-commit table -> provable"; else no "additive-only first-commit table -> provable"; fi

# 13. any non-added row on a first-committed table -> fail closed
reset; stub_count_issues=1
if diff_is_additive_only db H1 H2 "issues"; then no "non-added row on first-commit table -> quarantine"; else ok "non-added row on first-commit table -> quarantine"; fi

# 14. The two predicates must keep asking different questions. If this one ever
# narrows to removals, a table that exists in no pre-flatten commit could show
# modified rows and still read as proven.
reset; stub_count_issues=0
diff_is_additive_only db H1 H2 "issues" || true
if grep -q "diff_type <> 'added'" "$QUERY_LOG"; then
  ok "first-commit proof still counts every non-added row"
else
  no "first-commit proof still counts every non-added row (queries were: $(cat "$QUERY_LOG"))"
fi
